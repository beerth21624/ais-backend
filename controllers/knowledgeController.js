...
const updateKnowledge = async (req, res) => {
  ...
  try {
    await Knowledge.findByIdAndUpdate(id, updatedKnowledge, { new: true });
...
const deleteKnowledge = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id))
    return res.status(404).send(`No knowledge with id: ${id}`);
  try {
...
const createQaKnowledge = async (req, res) => {
  ...
  try {
    ...
    await FolderSchema.findByIdAndUpdate(folder_id, updatedFolder, {
      new: true,
    });
...
const updateQaKnowledge = async (req, res) => {
  ...
  try {
    const folder = await FolderSchema.findById(folder_id);
    const updatedFolder = {
      qa_knowledge: folder.qa_knowledge.map((qa) =>
        qa._id == qa_id ? { _id: qa_id, question, answer, record_status } : qa
      ),
    };
    await FolderSchema.findByIdAndUpdate(folder_id, updatedFolder, {
      new: true,
...
const deleteQaKnowledge = async (req, res) => {
  ...
  try {
    const folder = await FolderSchema.findById(folder_id);
    const updatedFolder = {
      qa_knowledge: folder.qa_knowledge.filter((qa) => qa._id != qa_id),
    };
    await FolderSchema.findByIdAndUpdate(folder_id, updatedFolder, {
      new: true,
    });
...
const updateImageKnowledge = async (req, res) => {
  ...
  try {
    const folder = await FolderSchema.findById(folder_id);
    const updatedFolder = {
      image_knowledge: folder.image_knowledge.map((image) =>
        image._id == image_id
          ? {
              _id: image_id,
              description: description,
...
const deleteImageKnowledge = async (req, res) => {
  ...
  try {
    ...
    CloudinayRemoveImage(
      folder.image_knowledge.find((image) => image._id == image_id).image_name
    );
    const updatedFolder = {
      image_knowledge: folder.image_knowledge.filter(
        (image) => image._id != image_id
      ),
    };
    await FolderSchema.findByIdAndUpdate(folder_id, updatedFolder, {
      new: true,
...
const deleteLinkKnowledge = async (req, res) => {
  ...
  try {
    const folder = await FolderSchema.findById(folder_id);
    const updatedFolder = {
      link_knowledge: folder.link_knowledge.filter(
        (link) => link._id != link_id
      ),
    };
    await FolderSchema.findByIdAndUpdate(folder_id, updatedFolder, {
      new: true,
...
const trainingLinkKnowledge = async (req, res) => {
  ...
  try {
    ...
    const link = await folder.link_knowledge.find(
      (link) => link._id == link_id
    );
    ...
    const updatedFolder = {
      link_knowledge: folder.link_knowledge.map((link) =>
        link._id == link_id
          ? {
              ...link,
              is_training: true,
              last_training: new Date(),
              content: sumParagraphs,
              name: headings[0] || link.name,
            }
          : link
      ),
    };
...
const multiTrainingLinkKnowledge = async (req, res) => {
  ...
  try {
    ...
    const updatePromises = link_ids.map(async (link_id) => {
      const link = folder.link_knowledge.find(
